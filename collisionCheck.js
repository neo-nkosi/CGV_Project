...
// for dikstra algorithm:
export function isTileWalkable(position, villaHouse, heightOffset = 0) {
    const rayVars = initializeRays();
    const direction = new THREE.Vector3(0, 0, -1); // Default direction (downwards)

    // Adjust position based on heightOffset
    position.y += heightOffset;

    // Set raycaster position based on the given position
    rayVars.raycaster.ray.origin.copy(position);
    rayVars.raycaster.ray.direction.copy(direction);

    const midBodyPosition = position.clone().add(new THREE.Vector3(0, 0.2, 0)); // roughly middle of a human body
    rayVars.middleRaycaster.ray.origin.copy(midBodyPosition);
    rayVars.middleRaycaster.ray.direction.copy(direction);

    // Check intersections with the villa
    const intersects = rayVars.raycaster.intersectObject(villaHouse, true);
    const midIntersects = rayVars.middleRaycaster.intersectObject(villaHouse, true);
    ...
    return !(checkCollision(intersects, position) || checkCollision(midIntersects, midBodyPosition));
}


export function checkDummyMovement(entity, obstacle) {
    const movementChecks = checkMovement(entity, obstacle);
